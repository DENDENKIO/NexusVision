# NEXUS Vision 仕様書 v4.2

**最終更新: 2026-04-28**
**ステータス: Phase 0/1/2/3/4/5 完了**
...
NDK 27（arm64-v8a）、CMake 3.22.1、C++17 / C11。libjpeg-turbo 3.1.4.1 を静的ライブラリとしてビルドし、ncnn-android-vulkan と共に `realesrgan_native` 共有ライブラリにリンク。手動構成ヘッダー（`jconfig.h`, `jconfigint.h`, `jversion.h`）で SIMD 無効・算術符号化有効・JPEG_LIB_VERSION=62 を設定。
...
### 5.5 ストリーミング JPEG 保存（実装済み）

libjpeg-turbo 3.1.4.1 をネイティブ層に組み込み、大画像（4096×4096 超）を OOM なしで保存する仕組みです。スキャンライン単位（64行バッチ）で RGBA→RGB 変換しながら JPEG に書き出します。

JNI API: `nativeJpegBeginWrite(fd, w, h, quality)` → `nativeJpegWriteRows(ctx, bitmap, startRow, numRows)` → `nativeJpegEndWrite(ctx)`。Kotlin 側: `saveBitmapToGallery()` が `width × height > 4096²` を検出すると自動的に `saveBitmapToGalleryStreaming()` へルーティング。品質: 95、出力先: `Pictures/NexusVision/`。MediaStore `IS_PENDING` フラグで書き込み中の不完全ファイル表示を防止します。

---

## 第6章 超解像パイプライン

### 6.1 Real-ESRGAN x4plus（現行ベースライン・実装済み）

NCNN Vulkan バックエンド。GPU で 3〜5 秒（小画像）、大画像はタイル分割で 3〜5 分。モデル: realesr-general-x4v3（約 2.4 MB param + bin）。NcnnSuperResolution クラスで 3-Stage Fusion Pipeline（AI + Guided Filter + DWT + IBP）を実装済み。128×128 タイル + 16px オーバーラップで処理し、4096×4096 出力上限。

### 6.2 NLM デノイズ

Non-Local Means。512×512 で約 500 ms。ノイズが多い場合のみ DEOR エントロピー判定で適用。

### 6.3 スマートデジタルズーム（実装済み）

チャットで「ズーム」「拡大」と送信 → 範囲選択 UI（CropSelector）で ROI 指定 → RegionDecoder で最大 4096px デコード → RouteCProcessor で超解像 → saveBitmapToGallery で保存。

### 6.4 明暗補正

線形ヒストグラムストレッチ（< 10 ms）。低光量時ガンマ補正 0.4〜0.6（< 5 ms）。ImageCorrector で実装済み。

---

## 第7章 EASS + FECS（NEXUS Vision オリジナル）
...
F の閾値（初期値 1.5 と 4.0）は Phase 15 で 100 枚のテスト画像を使い、PSNR / LPIPS / 処理時間の 3 指標でグリッドサーチにより最適化します。シーム補正のオーバーラップ幅も同時に検証します。テストを繰り返しながら最適値に昇華させます。
...
### 11.1 キュー管理 (BatchEnhanceQueue)

複数画像の URI をキューに登録し、1 枚ずつ順番に処理するためのシングルトン管理。QUEUED/PROCESSING/DONE/FAILED/PAUSED の 5 つの状態を持ち、StateFlow で全プロセスから進捗監視が可能。最大 20 枚までの同時投入に対応。

### 11.2 バックグラウンド処理 (BatchEnhanceWorker)

Jetpack WorkManager の `CoroutineWorker` を使用。フォアグラウンドサービス (`setForeground`) と連携し、アプリがバックグラウンドに移動したり KILL されても処理を継続。`RouteCProcessor` を内部で初期化し、ネイティブ層の `libjpeg-turbo` ストリーミング保存により大規模画像でも OOM を回避。処理間隔に 500ms のディレイを挿入し、継続的な負荷による急激な発熱を抑制。

### 11.3 サーマル・アダプティブ・制御

`ThermalMonitor` とリアルタイム連携。
- **SEVERE/CRITICAL**: 処理を一時停止 (PAUSED) し、10秒ごとに再チェック。温度低下で自動再開。
- **EMERGENCY**: 処理を中断 (ABORT) し、完了分のみ保存。

### 11.4 通知システム (BatchNotificationHelper)

- **進捗通知**: 残り時間の推定値 (ETE) を含む Ongoing 通知。
- **個別完了通知**: 1枚ごとの保存完了を通知。
- **一括完了通知**: 成功数/失敗数をまとめて通知。
- **発熱警告通知**: 一時停止・中断の理由を明示。

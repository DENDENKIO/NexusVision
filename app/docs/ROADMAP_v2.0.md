...
| Phase | 名称 | 期間 | ステップ数 | 主な成果物 |
|---|---|---|---|---|
| 1 | プロジェクト基盤 | 2週 | 6 | 新規プロジェクト作成、Gradle構成、Manifest、Application クラス |
| 2 | エンジン基盤 | 2週 | 6 | NexusEngineManager、EngineSocket、ThermalMonitor、フォアグラウンドサービス |
| 3 | DEOR + キャッシュ | 2週 | 6 | EntropyCalculator、PHashCalculator、AdaptiveResizer、L1/L2 キャッシュ、ObjectBox |
| 4 | 基本 UI + バッチ | 2週 | 5 | Compose メイン画面、画像選択、単一推論画面、WorkManager バッチ処理 |
| 5 | 100MP + ドキュメント鮮鋭化 | 2週 | 5 | DirectCrop100MP、DocumentSharpener、ML Kit OCR 統合 |
| 6 | EASS + FECS | 3週 | 7 | FECSScorer、EASSPipeline、タイル分割・ルート振り分け・結合、閾値テスト |
| 7 | Real-ESRGAN + 画像補正 | 2週 | 5 | NCNN JNI ブリッジ、RealEsrganBridge、NLM デノイズ、明暗補正 |
| 8 | ファイル解析 | 2週 | 4 | ExcelCsvParser、SourceCodeParser、PdfExtractor |
| 9 | OCR + 表復元 | 1週 | 3 | MlKitOcrEngine、TableReconstructor、CSV/Markdown 出力 |
| 10 | OS 統合 | 2週 | 6 | App Actions、Assist API、ACTION_PROCESS_TEXT、ACTION_SEND、クイック設定タイル、HUD |
| 11 | 長期記憶 | 1週 | 3 | HNSW ベクトル DB、意味的検索、パーソナライズ基盤 |
| 12 | ウィジェット + 通知 | 1週 | 3 | Jetpack Glance ウィジェット、通知インライン応答 |
| 13 | SDK + API | 1週 | 3 | NexusContentProvider、IntentApiReceiver、署名検証 |
| 14 | 将来最適化（実験） | 3週 | 5 | LoRA 推論適用、SwinIR 蒸留モデル差し替え、1 ステップ拡散 SR、NPU 自動選択 |
| 15 | テスト + 最適化 | 2週 | 4 | E2E テスト、パフォーマンスプロファイリング、EASS 閾値グリッドサーチ、リリースビルド |

**合計: 約 28 週（7 ヶ月）、65 ステップ**

---

## Phase 1 – プロジェクト基盤（2 週・6 ステップ）
...
**Step 2-4**: フォアグラウンドサービス `engine/NexusEngineService.kt` 作成。`FOREGROUND_SERVICE_SPECIAL_USE` で宣言。エンジンのライフサイクル管理。

**Step 2-5**: NexusEngineManager に Gemma-4 用の LiteRT-LM 接続を実装。Phase 0 の TestEngine のコードを基に、確定済み API シグネチャ（EngineConfig コンストラクタ、SamplerConfig Double 型、sendMessage オーバーロード、response.toString()）で書き直し。

**Step 2-6**: 統合テスト。モデルファイルを adb push 後、エンジン初期化 → テキスト推論 → 破棄のサイクルを検証。

---

## Phase 3 – DEOR + キャッシュ（2 週・6 ステップ）
...
**Step 3-4**: `cache/CacheEntities.kt` + ObjectBox 初期化。L1PHashCache エンティティ（hash: Long, result: String, timestamp: Long）。L2InferenceCache エンティティ（imageId: String, jsonResult: String, accessCount: Int, lastAccess: Long）。
...
## Phase 4 – 基本 UI + バッチ（2 週・5 ステップ）

**Step 4-1**: `ui/MainScreen.kt` 拡張。画像選択ボタン（`ActivityResultContracts.PickVisualMedia`）、結果表示領域、設定ボタン。

**Step 4-2**: `ui/MainViewModel.kt` 拡張。画像選択 → DEOR → キャッシュ照合 → 推論 → 結果表示の完全フロー。

**Step 4-3**: バッチ処理画面（複数画像選択）。進捗表示 UI。

**Step 4-4**: `worker/BatchProcessingWorker.kt` – WorkManager CoroutineWorker。前処理 → キャッシュ照合 → エントロピー昇順ソート → 直列推論 → キャッシュ格納 → 進捗通知。

**Step 4-5**: ThermalMonitor とバッチ処理の連携。SEVERE 以上でバッチ一時停止。

---

## Phase 5 – 100MP + ドキュメント鮮鋭化（2 週・5 ステップ）
...
**Step 6-3**: ルート A 実装 – バイキュービック補間。

**Step 6-4**: ルート B 実装 – Real-ESRGAN（RRDB）呼び出し（Phase 7 の JNI ブリッジのスタブ）。

**Step 6-5**: ルート C 実装 – 1 ステップ拡散モデル呼び出しスタブ（Phase 14 で実モデル接続）。

**Step 6-6**: タイル結合 + 8 px オーバーラップ線形ブレンディング。

**Step 6-7**: 合成テスト画像で EASS 全フロー検証。ルート振り分け比率、処理時間、結合画像の品質目視確認。

---

## Phase 7 – Real-ESRGAN + 画像補正（2 週・5 ステップ）

**Step 7-1**: NCNN Android SDK セットアップ。`libs/` に .so 配置。JNI ブリッジ C++ ファイル作成。CMakeLists.txt。

**Step 7-2**: `image/RealEsrganBridge.kt` – JNI ラッパー。realesrgan-x4plus モデルのロード・推論・解放。

**Step 7-3**: `image/ImageCorrector.kt` – NLM デノイズ、線形ヒストグラムストレッチ、ガンマ補正。

**Step 7-4**: EASS ルート B の実接続（Step 6-4 のスタブを RealEsrganBridge に差し替え）。

**Step 7-5**: 実機テスト。S200 で 4× 拡大の処理時間・品質検証。

---

## Phase 8 – ファイル解析（2 週・4 ステップ）
...
## Phase 12 – ウィジェット + 通知（1 週・3 ステップ）

**Step 12-1**: `widget/ProgressWidget.kt` – Jetpack Glance。バックグラウンドタスク一覧・進捗表示。
...
| 週 | マイルストーン |
|---|---|
| 2 | Phase 1 完了：プロジェクトビルド成功 |
| 6 | Phase 2-3 完了：エンジン + DEOR + キャッシュ動作 |
| 10 | Phase 4-5 完了：基本 UI + 100MP + OCR 動作 |
| 13 | Phase 6 完了：EASS + FECS 初回動作 |
| 15 | Phase 7 完了：Real-ESRGAN 統合 |
| 18 | Phase 8-9 完了：ファイル解析 + 表復元 |
| 20 | Phase 10 完了：OS 統合 |
| 22 | Phase 11-13 完了：記憶 + ウィジェット + SDK |
| 25 | Phase 14 完了：実験的最適化 |
| 28 | Phase 15 完了：リリース候補 |
